...
// Values from the cascade animation spec §4. `glowInset`/`glowAlpha` drive an
// inward GlowFilter (pixi-filters) standing in for the spec's "inset Npx @
// M%" border glow - background/border colors are exact, the glow rendering
// technique is an implementation choice since PixiJS has no native
// "inset box-shadow" primitive.
...
export const HEAT_CELL_STATES: Record<HeatRung, HeatCellState> = {
	...
	2: { background: 0x2c1608, border: 0x5e2e0c, glowInset: 14, glowAlpha: 0.22 },
